...
fenomenler[6].name = "Justin Bieber";
...
function profilListesi(fenomen) {
  ...
  for (let i = 0; i < yenifenomen.length; i++) {
    yenifenomen[i] = yenifenomen[i].profile;
    console.log(yenifenomen[i]);
  }
  return yenifenomen;
}
...
function fenomenEkle(fenomenler, number, profile, followers, posts, platform) {
  ...
  yeniArray.push(yenifenomen);
  return yeniArray;
}
const yeniArray = fenomenEkle(
  fenomenler,
  6,
  "Workintech",
  10000000,
...
function enFenomenler(array) {
  ...
  for (let key in array) {
    if (array[key]["Followers"] > 100000000) {
      enArray.push(array[key].profile);
    }
  }
  return enArray;
}
